...
_key_press_rec_iter = 0
_key_press_rec = []
_key_press_rec_label_uni = ["OP_1","Start_hold","End_hold","OP_2","End_asana"]
_key_press_rec_label_bil = ["Left - OP_1","Left - Start_hold","Left - End_hold","Left - OP_2/Right - OP_1","Right - Start_hold","Right - End_hold","Right - OP_2","End_asana"]


def add_key_press_rec(time1,rectime):
    global _key_press_rec_iter, _key_press_rec_label_uni, _key_press_rec_label_bil, _key_press_rec
    # time_data = time1 - rectime
    # print("_is_uni",_is_uni)
    if _is_uni == 1:
        if _key_press_rec_iter >= 4:
            return
        else:
            _key_press_rec.append([_key_press_rec_label_uni[_key_press_rec_iter],datetime.now()])
            _key_press_rec_iter+=1
            # print(_key_press_rec_iter)
            # print("done")

    else:
        if _key_press_rec_iter >= 7:
            return
        else:
            _key_press_rec.append([_key_press_rec_label_bil[_key_press_rec_iter],datetime.now()])
            _key_press_rec_iter+=1
            # print(_key_press_rec_iter)
            # print("done")
...
keyboard.add_hotkey('ctrl+shift+t',add_key_press_rec,args=(time.time(),recording_start_time),timeout=0.5,trigger_on_release=False)
...
class BodyGameRuntime(object):
    ...
    def write_key_press_rec(self):
        self.csv_writer(self.path + '/key_press_rec.csv', [["Label","Time elaspsed since start of recording(in sec) :-"]] + _key_press_rec)
    ...
    def startRecording(self):
        ...
        asana_data = open("asana_data.txt","rt")
        temp_asana_data = "\t"+ f"""
        PLease enter number corresponding to asana name:-
        ...
        print(temp_asana_data)

        aasanaID = int(input())
        while (aasanaID>asana_counter) and (aasanaID <1):
            print("Wrong selection!")
            aasanaID = int(input("Please enter again: "))
        asana_data.close()
        asana_data = open("asana_data.txt","rt")
        for x in asana_data:
            if int(x[0]) == aasanaID:
                aasanaID = str(x[3:len(x)-3])
        asana_data.close()
        apple = input("Press Y/y if the asana is UNILATERAL. Otherwise press N/n. ")
        while ((apple != 'Y') and (apple != 'y') and (apple != 'n') and (apple != 'N')):
            print("Wrong Input! Please enter again.")
            apple = input("Press Y/y if the asana is UNILATERAL. Otherwise press N/n. ")
        if ((apple=='Y') or (apple == 'y')):
            _is_uni = 1
        else:
            _is_uni = 0
        ...
        # codec_2 = cv2.VideoWriter_fourcc(*'MJPG')
        codec_x264 = cv2.VideoWriter_fourcc('M', 'P', 'E', 'G')
        codec_2 = cv2.VideoWriter_fourcc('M', 'P', 'E', 'G')
        # codec_x264 = cv2.VideoWriter_fourcc('X', 'V', 'I', 'D')
        # codec_2 = cv2.VideoWriter_fourcc('X', 'V', 'I', 'D')
        # codec_2 = cv2.VideoWriter_fourcc('I','4','2','0')
        self._key_press = []
        recording_start_time = time.time()
        _key_press_rec.append(['Baseline/Start_Asana',datetime.now()])
        _key_press_rec_iter = 0
        ...
        # self.keyboard_thread = Thread(target = keyboard_press_thread,args = [recording_start_time],daemon = True)
        self._video_color = cv2.VideoWriter(self.path + '/color.avi', codec_x264, 30, (self._kinect.color_frame_desc.Width, self._kinect.color_frame_desc.Height))
        self._video_depth = cv2.VideoWriter(self.path + '/depth.avi', codec_2, 30, (self._kinect.color_frame_desc.Width, self._kinect.color_frame_desc.Height))
        self._video_infrared = cv2.VideoWriter(self.path + '/infrared.avi', codec_2, 30, (self._kinect.color_frame_desc.Width, self._kinect.color_frame_desc.Height))
    ...
    def recordingButton(self, start_msg, stop_msg, x, y, w, h, start_col, stop_col):
        global _key_press_rec_iter, _key_press_rec_label_uni, _key_press_rec_label_bil, _key_press_rec, recording_start_time
        mouse = glfwSetCursorPosCallback(self._screen, cursor_position_callback)
        ...
        while not self._done:
            ...
            for event in pygame.event.get():  # User did something
                if event.type == pygame.QUIT:  # If user clicked close
                    ...
                    self._screen = pygame.display.set_mode(event.dict['size'], HWSURFACE | DOUBLEBUF | OPENGL, 32)
